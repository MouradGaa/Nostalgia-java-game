...
public class MenuState extends GameState {
    ...
    public void init() {
        //draw background
        bg = new ImageIcon(getClass().getResource("/bg.gif")).getImage();
        titleColor = new Color(151, 0, 0) ; // set color
        titleFont = new Font("Century Gothic",Font.BOLD,32) ; // set font
        font = new Font("Century Gothic", Font.BOLD,20) ;
        menu_color = new Color(0xFFD04C4C, true) ;
    ...
    public void render(Graphics2D g) {
        //draw background
        g.drawImage(bg,0,0, 215, 350,null);
        //draw title
        g.setColor(titleColor);
        g.setFont(titleFont);
        g.drawString("NOSTALGIA",20,160);
        //draw options
        g.setFont(font);
        if(help)
        {
        ...
        for(int i=0; i< options.length;i++)
        {
            ...
            g.drawString(options[i],85,190 + i *18);
        }
        }
    }
