...
public void render(Graphics2D g)
{
    ...
    for (int row =row_start; row< row_start + num_rows_draw; row++)
    {
        ...
        for(int col= col_start; col<col_start + num_cols_draw; col++)
        {
            ...
            int pos_rc = map[row][col] ;
            int r = pos_rc/num_tilesPerLine ; // let's say pos is 30 , r= 30/30 = 1 and c = 30%30 = 0
            int c = pos_rc % num_tilesPerLine ;  // ---> draw the tile in the 2nd row first column

            g.drawImage(tiles[r][c].getTile(),x + col*tile_size, y + row*tile_size, null);
